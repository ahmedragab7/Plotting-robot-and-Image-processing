...
for i=1:length(my_path)
    if my_path(i,2)<-5
      ...
      y=y+1;
    elseif (my_path(i,2)>-5 & my_path(i,2)<3)
...
for n = 1:length(new_path1)-1
    ...
    ind = ind + 1;
    hold off;
    plot( locationX,locationY,'r.','MarkerSize',10);
    %plot([new_path1(n,1),new_path1(n+1,1)],[new_path1(n,2),new_path1(n+1,2)],'r*'); 
    hold on;
    grid on;
    ...
    theta = getikine(roboArm, [new_path3(n,1), new_path3(n,2)],'left');
    P = getfkine(roboArm, theta - roboArm.offset);
